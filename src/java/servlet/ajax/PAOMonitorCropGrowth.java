 ...
 * @author RubySenpaii
 */
public class PAOMonitorCropGrowth extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        ArrayList<Municipality> barangays = (ArrayList<Municipality>) session.getAttribute("municipalities");
        for (int a = 0; a < barangays.size(); a++) {
            try {
                JSONObject barangay = new JSONObject();
                barangay.put("brgyName", barangays.get(a).getMunicipalityName());
                ...
                barangay.put("harvested", barangays.get(a).getHarvestingArea());
        ...
        try {
            ...
            System.out.println("data written for transfer to pao/monitorCropGrowth.jsp");
            response.getWriter().write("[" + jsonObjects.toString() + "]");
        } catch (JSONException ex) {
            Logger.getLogger(MAOMonitorCropGrowth.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
